...
import { supabase } from "../utils/supabase"; // パスを適切に調整してください
...
const questions = [
  ...
  {
    id: "q3",
    text: "Chat Display character settings",
    type: "single-select",
    options: ["English", "Romaji", "Katakana", "Kanji"],
  },
  {
    id: "username",
  ...
  const saveSurveyToSupabase = async () => {
    try {
      ...
      const surveyData: SurveyResponse = {
        age_group: answers.q1 || "",
        features: answers.q2 || [],
        display_level: answers.q3 || "",
      };

      const { error } = await supabase.from("survey_responses").insert([
        {
          user_id: user?.id || null, // 認証されていない場合はnull
          ...surveyData,
        },
      ]);
  ...
  const completeSurvey = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Supabase save failed, but continuing with local save:",
          supabaseError
        );
        // Supabaseの保存に失敗してもアプリの進行は続ける
      }
